...
export function initLiquidBg() {
  ...
  // Prism preset
  const c1 = parseColor("#FFFFFF");
  const c2 = parseColor("#66B3FF");
  const c3 = parseColor("#FFFFFF");
  ...
  const mount = new ShaderMount(canvas, warpFragmentShader, uniforms, 0.2, -299 * 10);

  // Pause when off-screen to save GPU
  const observer = new IntersectionObserver(([e]) => mount.setSpeed(e.isIntersecting ? 0.2 : 0), { threshold: 0.01 });
